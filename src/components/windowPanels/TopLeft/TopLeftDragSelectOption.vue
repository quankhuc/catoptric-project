...
import { forOptionActionKey, Option } from './TopLeftDragSelectCommon';
...
const topLeftDragSelectOptionRef = ref<HTMLElement>();

const option = computed<Option>(() => ({
  dom: topLeftDragSelectOptionRef.value as HTMLElement,
  value: props.value,
...
const optionClass = computed(() => ({
  'top-left-drag-select-option': true,
  'top-left-drag-select-option--selected': isSelected.value,
  'top-left-drag-select-option--disabled': props.disabled,
...
<template>
  <div ref="topLeftDragSelectOptionRef" :class="optionClass" @click="onClick" @pointerdown="onPointerDown">
    <slot />
  </div>
</template>
