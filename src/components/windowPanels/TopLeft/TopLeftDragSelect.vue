...
import { InnerDragSelectProps, forOptionActionKey, Option } from './TopLeftDragSelectCommon';
import { useClickToSelect, useDragToSelect } from './TopLeftDragSelectHook';
...
const dragSelectClass = computed(() => ({
  'top-left-drag-select': true,
  'top-left-drag-select--disabled': props.disabled,
}));
...
<template>
  <div ref="containerRef" class="top-left-drag-select__wrapper">
    <div ref="contentRef" :class="dragSelectClass" style="position: relative" @click="onContentRefClick">
      <slot />
      <div class="top-left-drag-select__area" :class="props.dragAreaClass" :style="areaStyle" />
    </div>
  </div>
</template>
