...
import { InnerDragSelectProps, forOptionActionKey, Option } from './RightMostDragSelectCommon';
import { useClickToSelect, useDragToSelect } from './RightMostDragSelectHook';
...
const dragSelectClass = computed(() => ({
  'right-most-drag-select': true,
  'right-most-drag-select--disabled': props.disabled,
}));
...
<template>
  <div ref="containerRef" class="right-most-drag-select__wrapper">
    <div ref="contentRef" :class="dragSelectClass" style="position: relative" @click="onContentRefClick">
      <slot />
      <div class="right-most-drag-select__area" :class="props.dragAreaClass" :style="areaStyle" />
    </div>
  </div>
</template>
